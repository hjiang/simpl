...
Atom::value_type Fn::Call(Interpreter* interpreter, const expr_list_t& exprs) {
  if (exprs.size() < 1) {
    throw std::runtime_error("fn: missing arguments");
  }
  auto i = exprs.begin();
  auto params = dynamic_pointer_cast<const Vector>(*i++);
  FnDef::param_list_t param_list;

  for (const auto& param : params->exprs()) {
    auto atom = dynamic_pointer_cast<const Atom>(param);
    param_list.push_back(atom->value<Symbol>().name);
  }
